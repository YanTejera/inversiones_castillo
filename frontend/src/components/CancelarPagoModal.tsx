...
const MOTIVOS_CANCELACION = [
  { value: 'pago_erroneo', label: 'Pago Erróneo', description: 'El monto o los datos del pago son incorrectos' },
  { value: 'cliente_equivocado', label: 'Cliente Equivocado', description: 'El pago fue asignado al cliente incorrecto' },
  { value: 'cancelado_por_cliente', label: 'Cancelado por el Cliente', description: 'El cliente solicitó la cancelación del pago' },
  { value: 'duplicado', label: 'Pago Duplicado', description: 'Se registró el mismo pago más de una vez' },
  { value: 'metodo_pago_invalido', label: 'Método de Pago Inválido', description: 'Problema con el método de pago utilizado' },
        ...
        <div className="p-6">
          ...
          <div className="bg-gray-50 p-4 rounded-lg mb-6">
            ...
            <div className="space-y-1 text-sm text-gray-600">
              <div>Pago ID: #{pago.id}</div>
              <div>Monto: {formatCurrency(pago.monto_pagado)}</div>
              <div>Fecha: {new Date(pago.fecha_pago).toLocaleDateString()}</div>
              <div>Método: {pago.tipo_pago_display || pago.tipo_pago}</div>
              {pago.venta_info && (
                <div>Venta: #{pago.venta_info.id}</div>
              )}
            </div>
          </div>
            ...
            <div className="flex items-start">
              ...
              <div className="text-sm">
                ...
                <p className="text-yellow-700">
                  Al cancelar este pago, el monto de {formatCurrency(pago.monto_pagado)} será 
                  devuelto al saldo pendiente del cliente. Esta acción quedará registrada 
...
export default CancelarPagoModal;